...
const CreateELD = () => {
  ...
  const handleMouseDown = (e) => {
    ...
    const snappedX = Math.round(x / X_GAP) * X_GAP;
    const snappedY = Math.round(y / Y_GAP) * Y_GAP;
  ...
  const handleSubmit = async () => {
    try {
      ...
      alert('Activity Logs created successfully!');
    } catch (error) {
    ...
    <div>
      ...
      >
        <Layer>
          ...
          {Array.from({ length: 24 }, (_, i) => (
            <Line key={`blue-line-${i}`} points={[i * 4 * X_GAP, 0, i * 4 * X_GAP, HEIGHT]} stroke="blue" strokeWidth={1} />
          ))}
          {Array.from({ length: X_STEPS / 2 }, (_, i) => (
            <Line key={`dotted-${i}`} points={[((i * 2) + 1) * X_GAP, 0, ((i * 2) + 1) * X_GAP, 0.5 * Y_GAP]} stroke="black" strokeWidth={1} />
          ))}

{Array.from({ length: X_STEPS / 3 }, (_, i) => (
            <Line key={`dottedz-${i}`} points={[((i * 3) + 1) * X_GAP, 0, ((i * 3) + 1) * X_GAP, 0. * Y_GAP]} stroke="black" strokeWidth={1} />
          ))}
          ...
          {currentLine.length > 1 && (
            <Line points={currentLine.flatMap(p => [p.x, p.y])} stroke="black" strokeWidth={2} />
          )}
          {ACTIVITIES.map((activity, i) => (
            <Text key={activity} x={5} y={(i + 0.5) * Y_GAP} text={activity} fill={COLORS[i]} fontSize={14} />
          ))}
        </Layer>
      </Stage>
